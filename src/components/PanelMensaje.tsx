      ...
      <div className="panel-chat">
        ...
        <div className="chat-header">
          <div className="chat-header-info">
            <div className="chat-avatar">{conversacion.empleado.charAt(0)}</div>
            <div>
              <h3>{conversacion.empleado}</h3>
              <p>
                {tipoInfo.emoji} {tipoInfo.label} — {conversacion.asunto}
              </p>
            </div>
          </div>
        ...
        <div className="chat-mensajes" ref={chatRef}>
          {conversacion.mensajes.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`chat-burbuja-contenedor ${esMio ? 'mio' : 'suyo'}`}>
                <div className={`chat-burbuja ${esMio ? 'burbuja-mia' : 'burbuja-suya'}`}>
                  <p className="chat-nombre">{esAdmin ? '👑 Administrador' : `👤 ${msg.de}`}</p>
        ...
        <div className="chat-input-contenedor">
          <textarea className="chat-input" placeholder="Escribe un mensaje... (Enter para enviar)" value={texto} onChange={(e) => setTexto(e.target.value)} onKeyDown={handleKeyDown} rows={2} />
          <button className="chat-btn-enviar" onClick={handleEnviar} disabled={texto.trim() === ''}>
            📨
          </button>
        </div>
      </div>
