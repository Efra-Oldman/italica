...
function TarjetaMiTarea({ tarea, onIniciar, onFinalizar }) {
  ...
  useEffect(() => {
    ...
    if (tarea.estado === 'en progreso' && tarea.timestampInicio) {
      ...
      const yaTranscurridos = Math.floor((Date.now() - tarea.timestampInicio) / 1000)
  ...
  return (
    <div className="tarjeta" style={{ borderLeft: `4px solid ${getBordePrioridad(tarea.prioridad)}` }}>
      ...
      <div className="fechas-tarea">
        <p>
          🟢 <strong>Inicio programado:</strong> {tarea.fechaInicio} {tarea.horaInicio}
        </p>
        <p>
          🔴 <strong>Fin programado:</strong> {tarea.fechaFin} {tarea.horaFin}
        </p>
      </div>

      <p style={{ color: '#666', fontSize: '13px', marginTop: '8px' }}>{tarea.descripcion}</p>
      ...
      {tarea.horaInicioReal && (
        <div className="fechas-tarea" style={{ marginTop: '8px' }}>
          <p>
            ▶️ <strong>Iniciada a las:</strong> {tarea.horaInicioReal}
          </p>
          {tarea.horaFinReal && (
            <p>
              ✅ <strong>Finalizada a las:</strong> {tarea.horaFinReal}
            </p>
          )}
