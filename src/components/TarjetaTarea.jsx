function TarjetaTarea({ tarea, onEliminar, onEditar, onCambiarEstado, onEvaluar, onAsignarIA }) {
  ...
  function renderEstrellas(calificacion) {
    return [1, 2, 3, 4, 5].map((e) => (
      <span key={e} style={{ color: e <= calificacion ? "#f57f17" : "#ddd", fontSize: "18px" }}>★</span>
    ))
  }
  ...
  return (
    <div className="tarjeta" style={{ borderLeft: `4px solid ${getBordePrioridad(tarea.prioridad)}` }}>
      ...
      <p>👤 <strong>Asignado a:</strong> {tarea.empleadoAsignado || "Sin asignar"}</p>

      <div className="fechas-tarea">
        <p>🟢 <strong>Inicio:</strong> {tarea.fechaInicio || "—"} {tarea.horaInicio || ""}</p>
        <p>🔴 <strong>Fin:</strong> {tarea.fechaFin || "—"} {tarea.horaFin || ""}</p>
      </div>
      ...
      {!tarea.empleadoAsignado && tarea.estado === "pendiente" && (
        <button
          className="btn-ia-asignar"
          onClick={() => onAsignarIA(tarea)}
        >
          🤖 Asignar con IA
        </button>
      )}
