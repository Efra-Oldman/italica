...
function Mensajes() {
  ...
  const { conversaciones, iniciarConversacion, marcarLeida, eliminarConversacion } = useMensajes()
  ...
  const misConversaciones = esAdmin ? conversaciones : conversaciones.filter((c) => c.empleado === usuarioActual.nombre)
  ...
  return (
    <div className="pagina">
      ...
      <div className="resumen-tareas">
        ...
        <div className="resumen-item">
          <span className="resumen-numero" style={{ color: '#8B0000' }}>
            {noLeidas}
          </span>
      ...
      {!esAdmin && (
        <div className="formulario-empleado">
          ...
          <div className="formulario-grid">
            ...
            <div className="campo">
              <label>Asunto</label>
              <input placeholder="Ej: Consulta sobre mi tarea" value={asunto} onChange={(e) => setAsunto(e.target.value)} />
            </div>
          </div>

          <div className="campo" style={{ marginBottom: '16px' }}>
            <label>Mensaje</label>
            <textarea className="textarea-descripcion" placeholder="Escribe tu mensaje aquí..." value={contenido} onChange={(e) => setContenido(e.target.value)} rows={3} />
          </div>
      ...
      {conversacionesFiltradas.length === 0 ? (
        ...
        <div className="mensajes-lista">
          {conversacionesFiltradas.map((conv) => {
            ...
            return (
              <div key={conv.id} className={`mensaje-item ${!conv.leido ? 'mensaje-no-leido' : ''}`} onClick={() => handleVerConversacion(conv)}>
                <div className="mensaje-item-izq">
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p className="mensaje-item-de">{esAdmin ? `👤 ${conv.empleado}` : '📨 Al administrador'}</p>
      ...
      {/* Panel chat */}
      {convSeleccionada && <PanelMensaje conversacion={conversaciones.find((c) => c.id === convSeleccionada.id)} usuarioActual={usuarioActual} onCerrar={() => setConvSeleccionada(null)} />}
    </div>
  )
}
