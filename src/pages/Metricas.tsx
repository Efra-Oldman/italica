...
function Metricas() {
  ...
  const mejorEmpleado = todasMetricas.filter((m) => m.metricas.totalEvaluadas > 0).sort((a, b) => b.metricas.calificacionPromedio - a.metricas.calificacionPromedio)[0]

  // Totales generales
  const totalTareasCompletadas = todasMetricas.reduce((sum, m) => sum + m.metricas.totalCompletadas, 0)
    ...
    <div className="pagina">
      ...
      <div className="lista">
        {todasMetricas.map(({ empleado, metricas }) => (
          <TarjetaMetrica key={empleado.id} empleado={empleado} metricas={metricas} />
        ))}
      </div>
    </div>
  )
