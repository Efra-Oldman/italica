...
function Perfil() {
  ...
  const miPerfil = empleados.find((e) => e.nombre.toLowerCase() === usuarioActual.nombre.toLowerCase())

  const misTareas = tareas.filter((t) => t.empleadoAsignado === usuarioActual.nombre)
  ...
  function handleFinalizar(idTarea) {
    const tarea = tareas.find((t) => t.id === idTarea)
    const ahora = new Date()
    const tiempoReal = Math.round((ahora.getTime() - tarea.timestampInicio) / 60000)
    editarTarea({
      ...tarea,
      estado: 'completada',
  ...
  return (
    <div className="pagina">
      <div className="perfil-header">
        <div className="perfil-avatar">{usuarioActual.nombre.charAt(0).toUpperCase()}</div>
        <div className="perfil-info">
          ...
          {miPerfil && (
            <div style={{ marginTop: '8px' }}>
              <button className="btn-solicitar-skill" onClick={() => navigate('/mensajes')}>
      ...
      {misTareas.length === 0 ? (
        ...
        <div className="lista">
          {misTareas.map((tarea) => (
            <TarjetaMiTarea key={tarea.id} tarea={tarea} onIniciar={handleIniciar} onFinalizar={handleFinalizar} />
          ))}
        </div>
      )}
    </div>
